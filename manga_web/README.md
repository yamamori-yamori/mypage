# マンガページエディタ 開発仕様書 Ver.4.4
copyright 2026 やまもりやもり

ローカルAI向け開発仕様書。現行実装（プロジェクトJSON `version: "1.3"` / アプリ仕様 Ver.4.4）を基準とする。
...
interface EffectObject {
  id: string;
  type: "effect";
  kind: string;                    // 主に concentration | speedLines（他 kind は後方互換）
  scope: string;                   // "panel" 等
  panelId: string | null;
  params: object;                  // lineCount, lengthRatio, direction, thickness, origin...
  transform: Transform;
  zIndex: number; locked: boolean; visible: boolean;
}
```

- 集中線: 三角形（トンガリ）、焦点ハンドル、太さ(%)、乱数シード固定。
- スピード線: 向き・長さ乱数・太さ(%)。
- 配置はコマ内のみ。
...
| `effect` | 6. エフェクト | effect | コマ内配置、集中線/スピード線中心 |
...
├ css/style.css
├ README.md                          本仕様（Ver.4.4）
├ MANUAL.html                        図解マニュアル（Ver.4.4）
...
`data-models` → `scene-graph` → `page-manager` → `command-stack` → selection → clipboard →  
...
*本ドキュメントは実装（`js/`）を正とする。現行アプリ仕様は **Ver.4.4**。差異があればコードを優先し、本 README を更新すること。*
